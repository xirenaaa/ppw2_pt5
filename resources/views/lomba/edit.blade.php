    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="max-w-4xl mx-auto bg-white rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div class="bg-gradient-to-r from-amber-500 to-orange-600 p-6 text-white">
                ...
                <p class="text-amber-100">Perbarui informasi lomba: {{ $lomba->nama_lomba }}</p>
            </div>
            ...
            <form action="{{ route('lomba.update', $lomba->id_lomba) }}" method="POST" enctype="multipart/form-data" class="p-8">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="nama_lomba" id="nama_lomba" 
                           ...
                           class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition @error('nama_lomba') border-red-500 @enderror"
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="penyelenggara_lomba" id="penyelenggara_lomba" 
                           ...
                           class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition @error('penyelenggara_lomba') border-red-500 @enderror"
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <select name="id_bidang" id="id_bidang" 
                                class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition @error('id_bidang') border-red-500 @enderror" 
                                required>
                            <option value="">Pilih Bidang</option>
                            @foreach($bidang_lombas as $bidang)
                                <option value="{{ $bidang->id_bidang }}" 
                                        {{ old('id_bidang', $lomba->id_bidang) == $bidang->id_bidang ? 'selected' : '' }}>
                                    {{ $bidang->nama_bidang }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="kategori_peserta" id="kategori_peserta" 
                                class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition @error('kategori_peserta') border-red-500 @enderror" 
                                required>
                            <option value="">Pilih Kategori</option>
                            @foreach($kategori_peserta as $kategori)
                                <option value="{{ $kategori }}" 
                                        {{ old('kategori_peserta', $lomba->kategori_peserta) == $kategori ? 'selected' : '' }}>
                                    {{ $kategori }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="lokasi" id="lokasi" 
                                class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition @error('lokasi') border-red-500 @enderror" 
                                required>
                            <option value="">Pilih Lokasi</option>
                            @foreach($lokasi_options as $lok)
                                <option value="{{ $lok }}" 
                                        {{ old('lokasi', $lomba->lokasi) == $lok ? 'selected' : '' }}>
                                    {{ $lok }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <input type="date" name="tgl_lomba" id="tgl_lomba" 
                               value="{{ old('tgl_lomba', \Carbon\Carbon::parse($lomba->tgl_lomba)->format('Y-m-d')) }}"
                               class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition @error('tgl_lomba') border-red-500 @enderror" 
                               required>
                        @error('tgl_lomba')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select name="status" id="status" 
                                class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition @error('status') border-red-500 @enderror" 
                                required>
                            <option value="available" {{ old('status', $lomba->status) == 'available' ? 'selected' : '' }}>
                                Tersedia
                            </option>
                            <option value="unavailable" {{ old('status', $lomba->status) == 'unavailable' ? 'selected' : '' }}>
                                Tidak Tersedia
                            </option>
                        </select>
                        @error('status')
                ...
                <div class="mb-6">
                    ...
                    <input type="url" name="link_daftar" id="link_daftar" 
                           value="{{ old('link_daftar', $lomba->link_daftar) }}"
                           class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition @error('link_daftar') border-red-500 @enderror"
                ...
                <div class="mb-6">
                    ...
                    <textarea name="deskripsi" id="deskripsi" rows="6" 
                              class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition @error('deskripsi') border-red-500 @enderror"
                              placeholder="Tulis deskripsi lengkap tentang lomba..." required>{{ old('deskripsi', $lomba->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    <label class="block text-gray-700 font-bold mb-2">
                        <i class="fas fa-image text-pink-500 mr-2"></i>Gambar Saat Ini
                    </label>
                ...
                <div class="mb-6">
                    <label for="gambar" class="block text-gray-700 font-bold mb-2">
                        <i class="fas fa-upload text-pink-500 mr-2"></i>Upload Gambar Baru
                        <span class="text-gray-500 text-sm font-normal">(Opsional, Max: 5MB)</span>
                    </label>
                    <input type="file" name="gambar" id="gambar" accept="image/*"
                           class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition @error('gambar') border-red-500 @enderror"
